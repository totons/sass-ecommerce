...
$brands = Brand::where('status', 1)
    ...
    $blogs = Blog::where('status', 1)
        ...
        $homeBanner = Banner::where(['status' => 1, 'category_id' => 12])
            ->select('id', 'image', 'link', 'title')
            ->orderBy('id')
            ->limit(1)
            ->get();

        $hotDealsBanners = Banner::where(['status' => 1, 'category_id' => 9])
            ->select('id', 'image', 'link', 'title')
            ->orderBy('id')
            ->limit(4)
            ->get();

        $offersBanners = Banner::where(['status' => 1, 'category_id' => 13])
            ->select('id', 'image', 'link', 'title')
            ->orderBy('id')
            ->limit(4)
            ->get();
        ...
        // Active verified vendors for home shop cards (প্রোডাক্ট না থাকলেও শপ দেখাবে; বেশি প্রোডাক্ট আগে)
    ...
    public function cartStore(Request $request)
    {
        ...
        if ($orderCount >= $limitQty) {
            // AJAX কার্ট: রিডাইরেক্ট ফলো করলে ফ্ল্যাশ ব্যবহারকারীর স্ক্রিনে আসে না — ফ্লাশ + reload হিন্ট দিন
            if ($request->ajax() || $request->wantsJson()) {
                session()->flash('show_order_limit_modal', true);
        ...
        if ($availableStock !== null) {
            ...
            if ($availableStock <= 0) {
                $stockMsg = 'এই পণ্যটি বর্তমানে স্টক আউট, অর্ডার করা যাবে না।';
                if ($request->ajax() || $request->wantsJson()) {
                    return response()->json([
                        'success' => false,
                        'message' => $stockMsg,
                ...
                Toastr::error($stockMsg, 'স্টক আউট!');
            ...
            if ($totalRequested > $availableStock) {
                $limitMsg = 'স্টকে যত আছে তার বেশি অর্ডার করা যাবে না। সর্বোচ্চ ' . $availableStock . ' টি নিতে পারবেন।';
                if ($request->ajax() || $request->wantsJson()) {
                    return response()->json([
                        'success' => false,
                        'message' => $limitMsg,
                ...
                Toastr::error($limitMsg, 'স্টক সীমা!');
                return redirect()->back()->withInput();
            }
        }
        ...
        // একই event_id ব্রাউজার পিক্সেল + CAPI ডিডুপের জন্য
        $atcEventId = 'atc_' . $product->id . '_' . bin2hex(random_bytes(6));
        ...
        // AJAX request হলে JSON return (Data Layer / Pixel tracking এর জন্য)
        if ($request->ajax() || $request->wantsJson()) {
            return response()->json([
                'success' => true,
                'id'      => $product->id,
                'tracking' => [
                    ...
                    'value'      => (float) ($finalPrice * $requestedQty),
	...
	public function brandIndex()
    {
        ...
        return view('frontEnd.layouts.pages.brand_index', compact('brands', 'generalsetting', 'seo'));
    }
    ...
    public function brand($slug, Request $request)
{
    ...
    {
        try {
            // JSON বডি (jQuery/sendBeacon) কখনও $request->input() এ না আসলে সরাসরি ডিকোড
            $payload = [];
            ...
            $validator = Validator::make($payload, [
                ...
                'items'         => 'nullable',
                'product_image' => 'nullable|string|max:2000',
                'product_link'  => 'nullable|string|max:2000',
            ...
            // একই ফোনে আপডেট (ঠিকানা বদলে নতুন রো বানানোর বাগ এড়ানো)
            ...
            Log::error('Incomplete order save failed: '.$e->getMessage(), ['trace' => $e->getTraceAsString()]);
            return response()->json([
                'status'  => 'error',
    ...
    public function shop(Request $request)
    {
        ...
        $products = $products->paginate(24);
        $categories = Category::where('status', 1)->get();
        return view('frontEnd.layouts.pages.shop', compact('products', 'min_price', 'max_price', 'categories'));
    }
    ...
    public function livesearch(Request $request)
    {
        ...
        return view('frontEnd.layouts.ajax.search', compact('products', 'keyword'));
    }

    public function search(Request $request)
    {
        $products = Product::select('id', 'name', 'slug', 'new_price', 'old_price', 'stock', 'category_id', 'sold')
            ->where('status', 1)
            ->where('approval_status', 'approved');
        if ($request->keyword) {
            $products = $products->where('name', 'LIKE', '%' . $request->keyword . "%");
        ...
        return view('frontEnd.layouts.pages.search', compact('products', 'keyword', 'min_price', 'max_price', 'categories'));
    }
