...
if (typeof fbq === 'function') {
    fbq('track', 'Search', { search_string: @json($keyword ?? '') });
}
if (typeof ttq !== 'undefined') {
    ttq.track('Search', { query: @json($keyword ?? '') });
}
@endpush
@push('css')
    ...
    <style>
        ...
        .subcategory-filter-name { margin: 0; font-size: 14px; font-weight: 500; color: #333; }
        .subcategory-filter-label input {
            position: relative;
            border: 2px solid {{ $generalsetting->primary_color ?? '#7e22ce' }};
            border-radius: 2px;
            background: none;
            cursor: pointer;
            margin: 0 .55em 0 0;
        ...
        .subcategory-filter-label input[type=checkbox]:checked {
            background-color: {{ $generalsetting->primary_color ?? '#7e22ce' }};
            opacity: 1;
        }
        ...
        @media (max-width: 767.98px) {
            ...
            .cust_according_body ul li a, .subcategory-filter-name { font-size: 13px; }
        }
    </style>
@endpush
    ...
    <div class="container">
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-6">
                            <div class="filter_sort">
                                ...
                                <div class="page-sort">
                                    <form action="{{ route('search') }}" method="GET" class="sort-form">
        ...
        <div class="row">
            <div class="col-sm-3 filter_sidebar">
                ...
                <form action="{{ route('search') }}" method="GET" class="attribute-submit">
                    <input type="hidden" name="keyword" value="{{ $keyword }}" />
                                        ...
                                        <ul class="space-y-3">
                                            @foreach ($categories as $category)
                                                <li class="subcategory-filter-list">
                                                    <label for="cat-{{ $category->id }}" class="subcategory-filter-label">
                                                        <input class="form-checkbox form-attribute" id="cat-{{ $category->id }}"
                                                            name="category" value="{{ $category->id }}" type="radio"
                                                            @if (request()->get('category') == $category->id) checked @endif />
                                                        <p class="subcategory-filter-name">{{ $category->name }}</p>
                                                    </label>
                                                </li>
                                            @endforeach
                                            <li class="subcategory-filter-list">
                                                <label for="cat-all" class="subcategory-filter-label">
                                                    <input class="form-checkbox form-attribute" id="cat-all"
                                                        name="category" value="" type="radio"
                                                        @if (!request()->get('category')) checked @endif />
                                                    <p class="subcategory-filter-name">All Categories</p>
                                                </label>
                                            </li>
                                        </ul>
                                    </div>
            ...
            <div class="col-sm-9">
                <section class="category-products-section">
                    <div class="category-products-head">
                        <h2 class="category-products-title">{{ $keyword ? 'Search: ' . $keyword : 'Search Results' }}</h2>
                    </div>
                    <div class="category-products-slider">
                        ...
                        @empty
                            <div class="w-100 text-center py-5">
                                <p class="text-muted">কোন প্রোডাক্ট খুঁজে পাওয়া যায়নি।</p>
